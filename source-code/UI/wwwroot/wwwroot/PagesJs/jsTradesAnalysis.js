    ...
    Events:
    {
        Init: function () {
            ...
            $(frmAnalyzePage.Ctrls.btn_AnalyseDemoFiles).click(function () {
                ...
                $('.customrulelnk').each(function ()
                {
                    let Id = $(this).attr('Id');
                    let ThreatType = $(this).attr('ThreatType');
                    let tText = $(this).text();
                    let pmdruletypekey = $(this).attr('pmdruletypekey');

                    let ref = $(this).attr('ref');
                    let refname = $(this).attr('refname');
                    let pmdrulesstixkey = $(this).attr('pmdrulesstixkey');


                    if (pmdruletypekey == 'PRLT-0002')  // PMD Rule
                    {
                        if (XpathRuleIds.includes(Id) == false)
                        {
                            //  $('#TradesCustomisedRulesTab').append('<p class="mt-2">  <img src="/assets/images/threat1.png" width="20" height="20" /><a role="button" class="ms-2 customrulelnkshow link-opacity-10" Id=' + Id + ' ThreatType=' + ThreatType + ' >' + tText + ' </a>  </p>');
                            $('#TradesCustomisedRulesTab').append('<p class="mt-2 customrulelnkshow" role="button" Id=' + Id + ' ThreatType=' + ThreatType + '  pmdrulesstixkey=' + pmdrulesstixkey + ' ><img src="/assets/images/threat1.png" width="20" height="20" /><a role="button" class="ms-2  link-opacity-10" Id=' + Id + ' ThreatType=' + ThreatType + '  pmdrulesstixkey=' + pmdrulesstixkey + ' >' + tText + ' </a>  </p>');
                            XpathRuleIds.push(Id);
                        }
                    }
                    if (pmdruletypekey == 'PRLT-0001')   // Java Rule
                    {
                        if (JavaRuleIds.includes(ref) == false)
                        {
                            JavaRuleIds.push(ref);
                            $('#TradesCustomisedRulesTab').append('<p class="mt-2 customrulelnkshow" role="button" Id=' + Id + ' ThreatType=' + ThreatType + '  pmdrulesstixkey=' + pmdrulesstixkey + ' ><img src="/assets/images/threat1.png" width="20" height="20" /><a role="button" class="ms-2  link-opacity-10" Id=' + Id + ' ThreatType=' + ThreatType + '  pmdrulesstixkey=' + pmdrulesstixkey + ' >' + tText + ' </a>  </p>');

                            let JavaRule = {};
                            JavaRule.RuleRef = ref;
                            JavaRule.RuleId = refname;
                            JavaRule.PMDRulesStixKey = pmdrulesstixkey;
                ...
                $('.traderulelnk').each(function ()
                {
                    let Id = $(this).attr('Id');
                    let ThreatType = $(this).attr('ThreatType');
                    let tText = $(this).text();

                    if (_TradeRulesIds.includes(Id) == false)
                    {
                        _TradeRulesIds.push(Id);
                        let DataCarrier = {};
                        DataCarrier.Id = $(this).attr('id');
                        DataCarrier.Type = $(this).attr('threattype');
                        _TradeRules.push(DataCarrier);

                        $('#TradesRulesTab').append('<p role="button" class="mt-2 traderulelnkshow" Id = ' + Id + ' ThreatType =' + ThreatType +' >  <img src="/assets/images/threat1.png" width="20" height="20" /><a role="button" class="ms-2  link-opacity-10">' + tText + ' </a>  </p>');
                ...
                if (
                    ...
                    if ($(frmAnalyzePage.Ctrls.chkPMD).prop("checked") == true) {
                        ...
                        if (XpathRuleIds != null && XpathRuleIds.length > 0)
                        {
                            $.each(XpathRuleIds, function (index, val)
                            {
                                let XpathRuleDetail = {};
                                XpathRuleDetail.RuleId = val;
                    ...
                    InferDemo._TradeRules = _TradeRules;
        ...
        EditPmdView: function (Id) {
            ...
            if (frmAnalyzePage.IDs.isCustomizedRequestView == false)
            { HttpRequestBody.MethodURL = frmAnalyzePage.URLs.fnGetPmdViewSingle; }
